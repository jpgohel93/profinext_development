				...
				<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
				   ...
				   @endif
					...
					<div class="toolbar" id="kt_toolbar">
						<!--begin::Container-->
						<div id="kt_toolbar_container" class="container mx-7 d-flex flex-stack">
							<!--begin::Page title-->
												...
												@can("role-read")
													@forelse ($roles as $role)
														<tr>
															...
															<td class="role-value-td"><a href='{{route('usersByRole',$role->name)}}' target="_blank">{{$role->name}}</a></td>
															<td class="role-value-td">{{$role->total}}</td>
															<td class="role-value-td">{{date("Y-m-d",strtotime($role->created_at))}}</td>
															<td class="text-end">
                                                                @canany(["role-write","role-delete"])
                                                                    ...
                                                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-auto py-4 min-w-125px" data-kt-menu="true">
                                                                        @can("role-write")
                                                                            <div class="menu-item px-2">
                                                                                <a href="{{route('editRoleForm',$role->id)}}" class="menu-link" data-id="{{$role->id}}">
                                                                                    Edit
                                                                                </a>
                                                                            </div>
                                                                        @endcan
                                                                        @can("role-delete")
                                                                            <div class="menu-item px-2">
                                                                                <a href="javascript:void(0)" data-id="{{$role->id}}" class="removeRole menu-link">
                                                                                    Delete
                                                                                </a>
                                                                            </div>
                                                                        @endcan
